...
GeoScratch is an ES module WebGPU library. The public entrypoint is `src/index.js`, with `src/scratch.js` kept as a compatibility shim. TypeScript declaration files (`*.d.ts`) live beside matching JavaScript modules. Shared primitives live in `src/core/`; geospatial helpers and tiling structures are in `src/geo/`; reusable mesh generators are in `src/geometry/`; WebGPU resources are in `src/gpu/`; loaders are in `src/loaders/`; postprocessing helpers are in `src/effects/`; and higher-level terrain code is in `src/applications/`. Vite examples are under `examples/`. Documentation and branding assets live in `docs/assets/`; example-only assets live beside their owning example; library-owned assets live beside their source module. Tests belong in `tests/`.
