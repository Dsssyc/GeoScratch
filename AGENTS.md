...
`TextureResource.resize()` is the lasting explicit size-only allocation-replacement primitive. It must preserve the complete non-size physical descriptor and logical identity, use create-before-swap failure atomicity, advance only `allocationVersion`, preserve `contentEpoch`, mark the replacement empty, invalidate allocation-scoped views, and must not wait for queue completion before destroying the old texture. Surface following remains explicit (`surface.resize(...)` then `texture.resize(surface.size)`); do not add hidden observers, size-provider callbacks, runtime texture scans, or a parallel replacement path.
