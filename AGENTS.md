...
`ExternalImageUploadCommand` is the explicit native representation of `GPUQueue.copyExternalImageToTexture()`; it must preserve queue-call capture time and must not lower through CPU pixel extraction or `writeTexture()`.

`TextureResource.resize()` is the lasting explicit size-only allocation-replacement primitive and the only public texture allocation transition. It must preserve the complete non-size physical descriptor and logical identity, keep identity/descriptor/lifecycle/provenance/physical-allocation facts in ECMAScript-private backing slots exposed through read-only getters, validate transient-attachment constraints before native creation, use create-before-swap failure atomicity, advance only `allocationVersion`, preserve `contentEpoch`, mark the replacement empty, invalidate allocation-scoped views, and must not wait for queue completion before destroying the old texture. Resource transition helpers must remain internal modules rather than object methods or package-entrypoint exports; TypeScript `private` fields that emit writable JavaScript properties are insufficient for these invariants. Surface following remains explicit (`surface.resize(...)` then `texture.resize(surface.size)`); do not add hidden observers, size-provider callbacks, runtime texture scans, or a parallel replacement path.
