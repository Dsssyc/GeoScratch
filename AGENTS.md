...
GeoScratch is an npm workspace repository. The publishable ES module WebGPU library lives in `packages/geoscratch/`; its public entrypoint is `packages/geoscratch/src/index.js`, with `packages/geoscratch/src/scratch.js` kept as a compatibility shim. TypeScript declaration files (`*.d.ts`) live beside matching JavaScript modules. Shared primitives live in `packages/geoscratch/src/core/`; geospatial helpers and tiling structures are in `packages/geoscratch/src/geo/`; reusable mesh generators are in `packages/geoscratch/src/geometry/`; WebGPU resources are in `packages/geoscratch/src/gpu/`; loaders are in `packages/geoscratch/src/loaders/`; postprocessing helpers are in `packages/geoscratch/src/effects/`; and higher-level terrain code is in `packages/geoscratch/src/applications/`. Vite examples are an independent consumer workspace under `examples/` and must import `geoscratch` instead of reaching into library source by relative path. Documentation and branding assets live in `docs/assets/`; architecture decision records live in `docs/decisions/`; forward-looking architecture vision docs live in `docs/vision/`; example-only assets live beside their owning example; library-owned assets live beside their source module. Tests belong in `tests/`.
...
Use `docs/vision/` for forward-looking design material that has not yet become an accepted ADR. Before changing the `scratch` graphics-kernel API, read `docs/vision/scratch-graphics-kernel.md` and the modular bilingual docs under `docs/vision/scratch-api/`. The `scratch-api` docs define the current target model: explicit async runtime, runtime/surface separation, logical resources with physical GPU versions, explicit readiness policies, explicit bind layouts with shader inspection only as a helper, stable pipelines plus executable `Command` objects, persistent `PassSpec` objects, and explicit frame order with dependency validation. During `0.x.x`, old `scratch` APIs are reference material rather than compatibility constraints unless a later ADR says otherwise.
