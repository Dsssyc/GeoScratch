...
The left branding is cropped from the original root `DayDream.png`. The globe is
...
Rendering at 2400 × 2400 before downsampling keeps the example's fixed-pixel
screen pattern, points, and links fine enough for a README-sized banner.
...
The banner is 800 × 400 with 600 frames over thirty seconds at 20 fps, looping
...
branding is unchanged. Only the globe receives a black-preserving RGB gamma lift
(`output = 255 × (input / 255)^0.8`) to bring its midtones closer to the original
PNG. Light smoothing reduces screen-pattern aliasing; the composite edges fade to
black to avoid rectangular crop seams. The composite is area-downsampled to the
README display size and WebP is encoded at quality 60 without GIF palette reduction
or additional dithering. Compression preserves all 600 frames and their 50 ms
duration, so it does not alter the rotation speed or particle timing.
...
the capture reports no page, network, or GPU failures. The original PNG remains
the source for the static branding.
