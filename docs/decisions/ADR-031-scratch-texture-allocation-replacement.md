...
The native maximum-mip calculation returns `1` for one-dimensional textures, so
Scratch rejects any one-dimensional descriptor whose `mipLevelCount` is not `1`
before native texture creation.
