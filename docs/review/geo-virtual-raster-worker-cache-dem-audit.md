...
| Network fetch and CPU decode shared one opaque concurrency ceiling. | Independently configured priority-aware network and decode budgets with active, queued, limit, and observed-maximum facts. | Scratch `TaskPhaseBudget` gates the separate Geo Worker `fetch` and `decode` operations without adding tile concepts to WorkerSystem. | Unit proof reaches two simultaneous network permits and one decode permit, cancels/reprioritizes queued work, and returns all counts to zero; headed DEM proof enforces limits 2 and 1. | Replaced |
