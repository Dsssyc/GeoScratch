...
| R16 | Queue completion can identify only the enclosing replayed submission family.<br>Queue completion 只能定位 enclosing replayed submission family。 | Per-link command-targeted `queue-completion` incidents and the `SubmittedWork.done` rejection wrapper. | Enclosing operation family | Complete |
