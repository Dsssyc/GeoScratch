...
The frame sequence is now:

1. data frontier compute;
2. render-patch lookup clear, render-root local-cell selection/culling, bounded balance,
...
| `terrainMeshShader` active path | Yes | Library terrain shader module | Geo `webMercatorTerrainWgslModule` plus example `shaders/terrain-presentation.wgsl` | Preserved, strengthened, and separated | Geo owns high-precision position/elevation, complete render-patch lookup, level-difference-one stitching, and built-in wireframe; the example owns only fragment presentation | Generated-WGSL unit proofs plus nonblank shaded and pitched wireframe browser proofs | Active depth and grayscale behavior remain unchanged |
