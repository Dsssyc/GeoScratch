...
`geo-virtual-raster-gpu-feedback.test.js` separately asserts N-1-or-earlier consumption, busy-slot backpressure, capacity overflow, stale decision/snapshot rejection, canonical demand dedupe, and generation-matched retire decoding. It must never return raw mapped bytes or an unbounded history.
