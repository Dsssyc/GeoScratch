...
Persistent buffer creation is one Promise-returning allocation operation. Scratch validates and normalizes the descriptor before issuing exactly one native allocation inside matching validation and out-of-memory scopes. After awaiting scope acknowledgement, Scratch rechecks runtime disposal and device loss immediately before construction. The logical buffer is registered only when that handoff remains valid; a failed or cancelled candidate never becomes a live resource.
...
A changed resize is a Promise-returning create-before-swap transaction. Scratch normalizes and validates the requested size, keeps the old allocation installed while the candidate's validation and out-of-memory scopes settle, then rechecks runtime disposal, device loss, and resource disposal immediately before commit. Only a still-valid handoff installs the acknowledged allocation, clears allocation-scoped views, advances `allocationVersion` once, sets `state = empty`, and destroys the old texture. The next successful content producer advances from the preserved `contentEpoch`. Any candidate failure or lifecycle cancellation leaves every old allocation fact installed; Scratch does not destroy first or wait for queue completion. A second changed resize while one is pending fails structurally.

Diagnostic texture footprint is logical, not physical residency. Width and height shrink at every mip. Array-layer count remains constant for `2d` array textures, while depth shrinks at every mip for `3d` textures; format blocks and sample count are then applied to each level.
