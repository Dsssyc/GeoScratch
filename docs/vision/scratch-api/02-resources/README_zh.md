...
`LayoutCodec` 从同一个不可变 `LayoutArtifact` 同步准备 CPU packing、WGSL
accessor、buffer-view contract 与 readback view:
...
Canonical recursive vocabulary 覆盖 `i32`、`u32`、`f32`、精确 IEEE
binary16 `f16`、全部合法 numeric vector、所有 floating matrix shape、递归 fixed
array 与 structure、final-member runtime-sized array、storage atomic、显式 member
`@align` / `@size`，以及 opaque fixed/runtime buffer root。`bool`、abstract
numeric、pointer、reference、texture、sampler 与 external texture 不是 host
layout field。

Public TypeScript grammar 只接受 WGSL 能放入 host memory 的 layout：fixed array
只能包含 fixed-footprint element，opaque buffer 只能是 root，runtime-sized array
只能是 root 或所在 structure 的 final member。Runtime validation 对 JavaScript
和动态来源 descriptor 执行相同规则。

`LayoutArtifact` 由 `extent` 判别:

- `FixedLayoutArtifact` 发布 `byteLength`、`stride` 和 fixed
  `minimumBindingSize`；
- `RuntimeLayoutArtifact` 发布 `fixedPrefixByteLength`、
  `minimumBindingSize`，以及 `runtimeTail` 或 opaque-buffer byte granularity；
  它绝不会虚构 fixed total byte length。

Runtime layout 需要具体 host range 时，caller 必须提供
`{ runtimeElementCount }`。`codec.byteLength()`、packing、writing、upload view、
readback view、`BufferRegion` witness 与 buffer-view contract 共用同一套 extent
计算。

每个 artifact 暴露两类独立 identity:
...
只有当每个 count、byte-size product、field offset/end、stride 与最终 alignment
round-up 都是非负 JavaScript safe integer，且每个生成的 WGSL `u32` constant
不超过 `0xffffffff` 时，layout lowering 才发布 artifact。Overflow 会通过
`SCRATCH_LAYOUT_UNSUPPORTED_FORMAT` 和结构化 arithmetic fact fail closed；
Scratch 绝不会发布内部自相矛盾的 artifact。

每个 `usageCompatibility` entry 都是包含 `compatible`、`reasons`、
`requiredDeviceFeatures`、`requiredLanguageFeatures` 与
`requiresMutableStorage` 的结构化事实。`f16` 派生 `shader-f16`；atomic 要求
mutable storage；vertex 与 immediate usage 会拒绝其 WGSL domain 无法表达的
type。Uniform compatibility 服从 artifact 中具名的
`capabilityContract.uniformLayout`:

- `portable` 应用 core uniform-address-space alignment constraint，不静默 repack
  通用 host-shareable ABI；
- `uniform_buffer_standard_layout` 保留该 ABI，并显式派生相应 language-feature
  requirement。

`codec.bufferView()` 为 `bufferView`、`bufferArrayView` 或 `bufferLength`
创建不可变 `LayoutBufferViewContract`。Contract 显式保留 source/target layout、
address space、access mode、pointer path、parameter-buffer chain、offset、length、
alignment、minimum type size 与全部派生 feature requirement。Uniform view 只能
read，workgroup view 只能 read-write，storage view 可以 read 或 read-write。
Function-parameter path 显式派生 `unrestricted_pointer_parameters`；所有 buffer
view 都派生 `buffer_view`。Program 与 command validation 消费同一个 contract，
因此 dynamic binding 不能绕过 layout minimum 或静态 required byte range。
