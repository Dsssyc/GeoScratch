...
The buffer `ReadbackCommand` path is implemented through Promise-only `createReadbackCommand()` / `readbackCommand()` factories. A command becomes visible only after one reusable staging slot is acknowledged, uses an explicit source `contentEpoch`, enters submission order through `SubmissionBuilder.readback(...)`, stages once at that position, and returns the associated `ReadbackOperation` through `result({ after })`. Direct `ReadbackOperation` accepts either a BufferRegion or an explicit texture subresource and can materialize owned logical bytes or return a bounded mapped staging lease. Ordered `ReadbackCommand` remains buffer-only. Finite pending-operation and padded-staging-byte budgets are runtime policy.
