...
`TextureResource.resize()` 是 resource-lifecycle operation，不是 `Command`、upload、copy 或 submission step。它不创建 encoder、queue action、resource-access entry、producer epoch 或 content write。size 变化时，它推进 `allocationVersion`，保留 `contentEpoch`，并把 replacement 标为 empty。

Pass spec 与 command 保留逻辑 resource，而不是某一个 physical texture。在 submit 或 encode 时，render attachment、texture upload、external-image upload、全部 texture copy direction 和 texture binding 会解析 current physical texture 或 current view。因此，只要 immutable descriptor 仍适配，resize 前创建的 command 可以继续复用。

这种复用不绕过 validation。Upload 与 copy command 会在 encoder 或 queue effect 前，根据当前 allocation 重新校验 mip、origin、extent、layer 与 sample constraint。Texture binding 会同时针对 current layer range 与 compatibility-mode effective texture binding dimension 重新校验 bind-layout view dimension；render attachment 会在 command encoder creation 前重新校验单个 2D mip/layer view，但不应用只属于 texture binding 的 dimension constraint。固定 required `contentEpoch` 仍然要求精确匹配：replacement 前后数字 epoch 相同，并不表示 empty 的新 allocation 已可读。
