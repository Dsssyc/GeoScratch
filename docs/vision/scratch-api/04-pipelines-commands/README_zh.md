...
静态 indexed count 必须带 `indexBuffer`; 静态 vertex count 禁止携带它。Indirect count 在没有 `indexBuffer` 时选择 `drawIndirect`，携带时选择 `drawIndexedIndirect`。Direct count 使用 WebGPU integer domain，并允许 zero-count no-op。
