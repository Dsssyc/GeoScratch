...
这种复用不绕过 validation。Upload 与 copy command 会在 encoder 或 queue effect 前，根据当前 allocation 重新校验 mip、origin、extent、layer 与 sample constraint。Texture binding 会重新校验 bind-layout view dimension；render attachment 会在 command encoder creation 前重新校验单个 2D mip/layer view。固定 required `contentEpoch` 仍然要求精确匹配：replacement 前后数字 epoch 相同，并不表示 empty 的新 allocation 已可读。
