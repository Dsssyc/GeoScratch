...
每个 resource read access 都保留 `declaredContentEpoch`，其中是 authored number 或 `'current-at-step'`；`contentEpochBefore` 与 `contentEpochAfter` 则始终是 resolved number。skipped command 不产生 access。fallback 只记录最终选中 fallback 自己的 declaration 与 resolved facts。ledger 绝不会用 resolved epoch 改写可复用 command。
...
原生 indexed 与 indirect command 使用和 shader resource 相同的 declared-read validation 路径。Vertex、index 与 indirect buffer 必须声明 exact numeric epoch 或 `'current-at-step'`。同一 submission 中的前序 upload 或 GPU command 可以产生该内容; 后续 fixed-function read 会进入 `SubmittedWork.resourceAccesses`，但不会推进 resource epoch。Indirect argument 内容保持为 GPU data，不会为了 scheduler validation 被复制到 CPU。
