...
`TextureResource.resize()` does not add a submission step。它是 immediate resource allocation lifecycle，不是 queue work。`SubmissionBuilder` 保存逻辑 pass、command 与 resource reference；若 texture 在 builder 构造后、`.submit()` 前 resize，preflight 与 encoding 会解析并校验 submission 时的 current allocation。Texture-backed color 与 depth/stencil attachment 会显式选择一个 `2d` mip-level array layer；过期的 mip/layer view descriptor、不匹配的 current render extents/sample counts，或不再允许 `2d` 的 compatibility-mode effective texture binding dimension，都会在 command encoder creation 或 ledger mutation 前失败。
