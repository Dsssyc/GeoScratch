...
Default retention is 256 operation records, 32 incidents, and 256 KiB of serialized evidence; all are finite and configurable. An operation capacity of zero disables successful-operation history without disabling current facts or failure handling. The retained-byte counter measures deterministic JSON evidence, not JavaScript heap size.
