...
@group(2) @binding(0) var srcTexture: texture_2d<f32>;
@group(2) @binding(1) var dstTexture: texture_storage_2d<rgba16float, write>;
...
fn cMain(@builtin(global_invocation_id) id: vec3u) {
    ...
    for (var index: u32 = 1u; index <= staticUniform.steps; index++) {
        ...
        color += (
            linearLoad(srcCoords + vec2f(f32(index), 0.0), srcSize).rgb +
            linearLoad(srcCoords - vec2f(f32(index), 0.0), srcSize).rgb
        ) * weight;
        weightSum += 2.0 * weight;
    }
    textureStore(dstTexture, vec2i(id.xy), vec4f(color / weightSum, 1.0));
}
