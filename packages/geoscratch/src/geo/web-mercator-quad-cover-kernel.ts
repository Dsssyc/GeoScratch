...
import type { WebMercatorQuadCoverPolicy } from './web-mercator-quad-cover.js'
/** @internal */
export type CoverKernelDomain = Readonly<{
...
// Scalar projection uses JS f64 over the same f32 camera facts as the terrain
// vertex ABI. Exact integer identity and compensated limb subtraction precede
// local f32 projection. Candidate and metric enclosures follow ADR-127.
...
/** @internal Bounded numerical workspace; callers must snapshot a certified result. */
export class WebMercatorQuadCoverKernel {
    policy: WebMercatorQuadCoverPolicy & {
        lookupCapacity: number
    }
    domain: CoverKernelDomain
    lookup: Uint32Array
    patches: Uint32Array
    marks: Uint32Array
    state: Uint32Array
    ...
    metrics = 0
    visibilityTests = 0
    constructor(domain: CoverKernelDomain, policy: WebMercatorQuadCoverPolicy, lookupCapacity: number) {
        this.domain = domain
    ...
    find(l: number, r: number, c: number) {
        const mask = this.policy.lookupCapacity - 1, h = this.hash(l, r, c)
    ...
    split(i: number) {
        ...
        const p = this.get(i), l = p.level + 1, r = p.row * 2, c = p.col * 2
        this.set(i, l, r, c)
        this.set(this.n++, l, r, c + 1)
        this.set(this.n++, l, r + 1, c)
        this.set(this.n++, l, r + 1, c + 1)
    ...
    bounds(level: number, row: number, col: number): Bounds {
        ...
        let vertical = this.domain.verticalRange
        ...
        const z = (value: number) => {
            const h = F(value), ch = this.meta.cameraHigh[2], cl = this.meta.cameraLow[2]
            if (h === ch)
                return F(-cl)
            const d = F(h - ch), bridge = F(d - h), round = F(F(h - F(d - bridge)) - F(ch + bridge))
            return F(d + F(F(round + 0) - cl))
        }
    ...
    numerator(x: number, y: number, dx: number[], dy: number[]) {
        const px = this.viewport[0] * .5, py = this.viewport[1] * .5, ax = (dx[0] - x * dx[3]) * px, ay = (dx[1] - y * dx[3]) * py, bx = (dy[0] - x * dy[3]) * px, by = (dy[1] - y * dy[3]) * py
        const scale = Math.max(Math.abs(ax), Math.abs(ay), Math.abs(bx), Math.abs(by))
        if (!scale)
            return 0
        if (!(scale < 2 ** 110))
            return HUGE
        const a = ax / scale, b = ay / scale, c = bx / scale, d = by / scale, xx = a * a + b * b, xy = a * c + b * d, yy = c * c + d * d
        return scale * Math.sqrt(Math.max(0, .5 * (xx + yy + Math.sqrt(Math.max(0, (xx - yy) ** 2 + 4 * xy * xy)))))
    }
