...
describe('Flow Field viewport history', () => {
    ...
    it('reverse gathers and finitely decays raw history without temporal destruction', () => {
        ...
        expect(shader).to.include('floor(255.0 * faded * cleanupUniform.trailDecay) / 255.0')
        expect(shader).to.include('min(cleanupUniform.decaySteps, 3u)')
        expect(shader.indexOf('if (cleanupUniform.decaySteps == 0u) { return color; }')).to.be.lessThan(shader.indexOf('residual <= cleanupUniform.trailCutoff'))
