...
describe('Flow Field renderer composition', () => {
    ...
    it('orders publication support simulation contour history and submission explicitly', () => {
        ...
        const producer = source.indexOf('content = contentBuilder => {')
        const particles = source.indexOf('particles.encode(contentBuilder, particleSpawn.bindings, prepared, view,')
        const contour = source.indexOf('encodeContour(contentBuilder)', particles)
        ...
        expect(producer).to.be.greaterThan(spawn)
        expect(particles).to.be.greaterThan(producer)
        ...
        expect(source).to.include('contour.encode(contentBuilder, candidates')
        expect(source).to.include('encodeContour(builder)')
        expect(source).not.to.include('particles.encode(builder,')
    ...
    it('keeps complete-view readiness separate from reset-safe local particle work', () => {
        ...
        expect(source).to.include("const particlesEligible = framePresentation.view === 'particles' &&")
        expect(source).to.include('visualClock.tick(wallTime,particlesEligible && timeline.playing)')
        expect(source).to.include('const particlesAdvancing = particlesEligible && visualTime.referenceSteps > 0')
        expect(source).to.include('particlesAdvancing ? [particleRender.draw] : []')
        ...
        expect(source).to.include('content = presentationReady ? [inspector.encode(')
    })
    ...
    it('borrows optional pages through observation and reuses only their completed spatial plan', () => {
        ...
        expect(source).to.include('const observingPrefetch = observing.then(() => {')
    ...
    it('uses only velocity-derived products and public package entrypoints', () => {
        ...
        expect(source).to.include('permits one construction and at most two frames in flight')
        expect(source).to.include('await drainFrames()')
        expect(source).to.include('await sourceAcknowledgement')
