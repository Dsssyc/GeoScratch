...
describe('Flow Field velocity source contract', () => {
    ...
    it('keeps only the schema-two sampleKey frontend data path', () => {

        const dataset = read('examples', 'flowField', 'flow-dataset.ts')
        const source = read('examples', 'flowField', 'velocity-source.ts')
        const temporal = read('examples', 'flowField', 'temporal-velocity-raster.ts')
        const provenance = read('examples', 'flowField', 'flow-frame-provenance.ts')
        const frontend = `${dataset}\n${source}\n${temporal}\n${provenance}`

        expect(frontend).to.not.match(
            /loadFlowDatasetManifest|FlowDatasetManifest|createVelocityTime|FlowVelocityTime/
        )
        expect(frontend).to.not.match(
            /createTemporalVelocityRaster|TemporalVelocitySnapshot|framesPerTime|prefetch/i
        )
        expect(source).to.include('createVelocitySampleRuntime')
        expect(dataset).to.include('loadFlowFieldDataset')
    })
...
function candidateDescriptor(source, page, buffer) {
    ...
    const expected = source.resolvePage(page)
    return Object.freeze({
        ...
        cacheAddress: virtualRasterCacheAddress({
            ...
            plane: `velocity.${source.sampleKey}`,
            coherence: { mode: 'immutable', contentVersion: source.model.sourceRevision },
        ...
        url: expected.url,
        contentVersion: source.model.sourceRevision,
