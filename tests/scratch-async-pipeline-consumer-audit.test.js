...
describe('scratch async pipeline consumer audit', () => {
    ...
    it('awaits every ordinary Scratch render consumer and classifies legacy calls', () => {
        ...
        for (const relativePath of sourceFiles('tests', 'examples')) {
            ...
            visitCalls(sourceFile, (call) => {
                ...
                if (method !== 'createRenderPipeline' && method !== 'renderPipeline') return
                ...
                if (method === 'renderPipeline' && receiver === 'scr') {
                    legacyCalls.push(reference)
                    if (!legacyRenderFiles.has(relativePath)) {
                        violations.push(`${reference} is an unclassified legacy render call`)
                    }
                    if (isAwaited(call)) {
                        violations.push(`${reference} silently rewrites a legacy render call`)
                    }
                    return
                }

                if (!isAwaited(call) && !intentionalRenderPromiseFiles.has(relativePath)) {
                    violations.push(`${reference} consumes a Scratch render pipeline without await`)
                }
            })
        }
