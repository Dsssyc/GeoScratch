...
import { plane as legacyPlane } from '../packages/geoscratch/src/geometry/plane/plane.js'
import { sphere as legacySphere } from '../packages/geoscratch/src/geometry/sphere/sphere.js'
import {
...
const cases = [
    ...
    },
    {
        ...
        legacy: () => legacySphere(1, 8, 4, .2, 3, .1, 1.7),
...
describe('scratch geometry parity', () => {
    ...
    for (const testCase of cases) {
        it(`preserves ${testCase.label} byte facts`, () => {
            ...
            const legacyHash = hash(testCase.legacy())
            const scratchHash = hash(testCase.scratch())

            expect(legacyHash).to.equal(testCase.expected)
            expect(scratchHash).to.equal(testCase.expected)
            expect(scratchHash).to.equal(legacyHash)
        })
    }
})
