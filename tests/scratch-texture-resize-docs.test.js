...
describe('scratch texture resize documentation', () => {
    ...
    it('documents downstream resolution, provenance, and stable diagnostics', () => {
        ...
        expect(commands).to.include('Submission may create submission-scoped attachment views')
        expect(commands).to.include('never creates a persistent binding')
        expect(commands).to.include('calls `prepare()`, waits, retries, or repairs stale binding')
